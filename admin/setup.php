...
if(file_exists("../core/config.php")){
	die(_("The config file already exists! Please delete this file! You can also delete the config file to redo the setup."));
}
...
if(isset($_POST)&&!empty($_POST)){
	if(isset($_POST['db_name'])&&isset($_POST['db_user'])&&isset($_POST['db_pw'])&&isset($_POST['db_host'])&&isset($_POST['admin_user'])&&isset($_POST['admin_pw'])){
		if(!empty($_POST['db_name'])&&!empty($_POST['db_user'])&&!empty($_POST['db_pw'])&&!empty($_POST['db_host'])&&!empty($_POST['admin_user'])&&!empty($_POST['admin_pw'])){
			...
			if($db->connect_errno){
				$error=_("Please check your database data");
			}else{
...
<!DOCTYPE html>
<head>
	<title>Grades - <?php echo _("Setup"); ?></title>
...
<body>
	<h1>Grades - <?php echo _("Setup"); ?></h1>
	<form action="/admin/setup.php" method="post">
		<input type="text" name="db_name" placeholder="<?php echo _("Database Name"); ?>" />
		<br/><br/>
		<input type="text" name="db_user" placeholder="<?php echo _("Database Username"); ?>" />
		<br/><br/>
		<input type="password" name="db_pw" placeholder="<?php echo _("Database Password"); ?>" />
		<br/><br/>
		<input type="text" name="db_host" placeholder="<?php echo _("Database Host, 'localhost' in most cases"); ?>" />
		<br/><br/>
		<input type="text" name="admin_user" placeholder="<?php echo _("Admin Username"); ?>" />
		<br/><br/>
		<input type="password" name="admin_pw" placeholder="<?php echo _("Admin Password"); ?>" />
		<br/><br/>
		<input type="submit" value="<?php echo _("Setup");?>"/>
	</form>
